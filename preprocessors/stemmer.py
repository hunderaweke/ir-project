class PorterStemmer:
    ...
    def cvc(self, word):
        if len(word) >= 3:
            ...
            third = word[t]
            if (
                self.isConsonant(word, f)
                and self.isVowel(word, s)
    ...
    def step1b(self, word):
        ...
        if flag:
            if word.endswith("at") or word.endswith("bl") or word.endswith("iz"):
                ...
                self.doubleCons(word)
                and not self.endsWith(word, "l")
                and not self.endsWith(word, "s")
                and not self.endsWith(word, "z")
            ):
                word = word[:-1]
    ...
    def step4(self, word):
        if word.endswith("al"):
            ...
            base = word[:result]
            if self.getM(base) > 1 and (
                self.endsWith(base, "s") or self.endsWith(base, "t")
            ):
                word = base
